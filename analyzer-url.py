...
def plot_graph(all_work_by_date_range, date_ranges):
    ...
    plt.bar(periods, total_amounts, color='skyblue')
    for i, (period, jobs) in enumerate(zip(periods, total_jobs)):
        plt.text(i, total_amounts[i] + 1000, f"Total Jobs: {jobs}", ha='center', va='bottom', rotation=00)
    for i, amount in enumerate(total_amounts):
        plt.text(i, amount + 200, f"Total Amount: {amount} Shekel", ha='center', va='bottom', rotation=00)
    plt.xlabel('Date Range')
    plt.ylabel('Total Amount (Shekel)')
